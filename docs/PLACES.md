...
(`type: map`), which can only draw entities that carry coordinates – street areas, as drawn
by the custom card (`dist/voesendorf-waste-card.js`), are not possible there.
